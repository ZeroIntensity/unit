...
void
register_symbol(const std::string &name, void *address)
{
...
{
    return reinterpret_cast<Function>(UNIT_ExecutableBuffer_GetPointer(
                                          buffer))(args ...);
}
...
add_op_int(OpCode op, std::int64_t oparg)
{
    if (UNIT_FAILED(UNIT_Procedure_AddOperation(&procedure,
                                                static_cast<UNIT_OperationCode>
                                                (op),
                                                oparg))) {
...
{
    if (UNIT_FAILED(UNIT_Procedure_AddCallName(&procedure,
                                               name.c_str(),
                                               nargs))) {
        throw error(procedure.context);
    }
...
{
    if (UNIT_FAILED(UNIT_Procedure_AddCallProcedure(&procedure,
                                                    subprocedure.raw(),
                                                    nargs))) {
        throw error(procedure.context);
    }
...
{
    UNIT_JumpLabel *label = UNIT_Procedure_CreateJumpLabel(&procedure,
                                                           name.c_str());
    if (label == NULL) {
        throw error(procedure.context);
    }
...
{
    if (UNIT_FAILED(UNIT_Procedure_AddJump(&procedure,
                                           static_cast<UNIT_OperationCode>(
                                               opcode),
                                           label.raw()))) {
        throw error(procedure.context);
