...
create_artificial_instruction(_UNIT_Vector *instructions,
                              UNIT_OperationCode opcode,
                              int64_t oparg)
{
    assert(instructions != NULL);

    _UNIT_Operation *operation = _UNIT_Alloc(instructions->context,
...
UNIT_Procedure_OptimizeFold(UNIT_Procedure *procedure)
{
    ...
    if (UNIT_FAILED(_UNIT_Vector_Init(&optimized,
                                      context,
                                      size,
                                      _UNIT_Dealloc))) {
        return -1;
    ...
    typedef struct {
        enum { STACK_UNKNOWN, STACK_CONSTANT, STACK_LOCAL, STACK_ADDRESS,
               STACK_COMPARE } kind;
        int64_t value;
    } StackEntry;
    ...
    #define POP()                                   \
            (stack_depth > 0 ? stack[--stack_depth] \
                             : (StackEntry){ STACK_UNKNOWN, 0 })

    #define PEEK()                                    \
            (stack_depth > 0 ? stack[stack_depth - 1] \
                             : (StackEntry){ STACK_UNKNOWN, 0 })
    ...
    #define ADD_NEW_INSTRUCTION(inst, op)                             \
            did_change = 1;                                           \
            if (UNIT_FAILED(create_artificial_instruction(&optimized, \
                                                          inst,       \
                                                          op))) {     \
                goto error;                                           \
            }
    ...
    #define CONTINUE_AND_DISCARD()                      \
            did_change = 1; _UNIT_Dealloc(context, op); \
            continue
    ...
    for (UNIT_Size index = 0; index < size; ++index) {
        ...
        switch (op->instruction) {
            ...
            case UNIT_OP_POP: {
                POP();
                _UNIT_Operation *previous = _UNIT_Vector_GET(&optimized,
                                                             _UNIT_Vector_SIZE(
                                                                 &optimized) -
                                                             1);
...
static int8_t
should_inline(const UNIT_Procedure *target,
              const UNIT_Procedure *caller)
{
    assert(target != NULL);
    assert(caller != NULL);
    if (target == caller) {
              ...
              _UNIT_Vector *output)
{
    ...
    snprintf(return_name,
             sizeof(return_name),
             "_inlined_%s_return",
             target->name);
    UNIT_JumpLabel *end_label = UNIT_Procedure_CreateJumpLabel(procedure,
                                                               return_name);
    if (end_label == NULL) {
        return _UNIT_FAIL;
    }
    ...
    snprintf(return_local_name,
             sizeof(return_local_name),
             "_inlined_%s_return_value",
             target->name);
    ...
    for (UNIT_Size index = 0; index < size; ++index) {
        ...
        _UNIT_Operation *remapped = _UNIT_Alloc(context,
                                                sizeof(_UNIT_Operation));
        if (remapped == NULL) {
            return _UNIT_FAIL;
        }
...
UNIT_Procedure_OptimizeInline(UNIT_Procedure *procedure)
{
    ...
    for (UNIT_Size index = 0; index < size; ++index) {
        ...
        UNIT_Size local_offset =
            _UNIT_Vector_SIZE(&procedure->_local_variables);
        UNIT_Size label_offset = _UNIT_Vector_SIZE(&procedure->_jump_labels);

        if (UNIT_FAILED(copy_locals(&target->_local_variables,
                                    &procedure->_local_variables))) {
            goto error;
        }
        ...
        UNIT_Size string_offset =
            _UNIT_Vector_SIZE(&procedure->_global_strings);
        ...
        if (UNIT_FAILED(remap_offsets(procedure,
                                      target,
                                      &offsets,
                                      &optimized))) {
            goto error;
...
static UNIT_Status
gather_local_info(_UNIT_Vector *instructions,
                  LocalInfo **info_ptr)
{
    ...
    for (UNIT_Size index = 0; index < size; ++index) {
        ...
        switch (op->instruction) {
            ...
            case _UNIT_OP_STORE_LOCAL_NAME: {
                ...
                if (index > 0) {
                    _UNIT_Operation *previous = _UNIT_Vector_GET(instructions,
                                                                 index - 1);
    ...
    for (UNIT_Size index = 0; index < size; ++index) {
        ...
        switch (op->instruction) {
            ...
            case _UNIT_OP_STORE_LOCAL_NAME: {
                ...
                _UNIT_Operation *next = _UNIT_Vector_GET(instructions,
                                                         index + 1);
                if ((next->instruction == UNIT_OP_LOAD_LOCAL ||
                     next->instruction == _UNIT_OP_LOAD_LOCAL_NAME)
                    ...
                    // Skip the load
                    ++index;
                    _UNIT_Dealloc(context,
                                  _UNIT_Vector_STEAL(instructions, index));
                    continue;
                }
            ...
            case _UNIT_OP_LOAD_LOCAL_NAME: {
                ...
                if (info[local_index].store_count == 1
                    ...
                    // Propagate constant
                    ADD_NEW_INSTRUCTION(UNIT_OP_LOAD_INTEGER,
                                        info[local_index].constant_value);
                    CONTINUE_AND_DISCARD();
                }
